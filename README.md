...
- **프로젝트 워크스페이스**: 작품별 독립된 집필 환경 관리 및 미존재 시 새 프로젝트 생성 Empty State 제공
- **캐릭터 보드**: 인물 간의 관계, 욕망, 갈등 구조를 시각화한 인터랙티브 보드 및 핵심 인물 고정(📌) 기능을 통해 AI 관계도 재생성 시 소실 방지
- **시나리오 타임라인**: 주요 사건의 흐름을 관리하고 시각화
- **시놉시스 빌더**: 수집된 영감과 캐릭터 정보를 바탕으로 AI와 협업하여 시놉시스 초안 생성

### 4. 세계관 설정실 (World-Building)
- **시공간 무대**: 시간(Era)과 공간(Place)을 통합하여 무대를 계층형 트리(상위-하위)로 입체적 설계
- **세력 및 집단**: 드라마 속 주요 세력의 이념, 목표, 규모 및 조직 구조 관리
- **사회 제도 / 문화 관습**: 작중 법령/규칙(`RULE`)이나 관습/현상(`CULTURE`)을 연동된 범위 내에서 정의
- **작품 용어 사전**: 고유 용어 및 특수 은어를 인라인 즉시 편집 그리드 및 가나다 정렬 뷰로 최적화하여 작성
- **세계관 자유 메모**: 격식 없는 단상이나 백스토리를 자유롭게 작성

### 5. 내보내기 (Export)
- 작성된 프로젝트 정보를 텍스트, PDF 등 다양한 형식으로 내보내기 (Alpine Docker 한글 폰트 로드 버그 수정 완료)
