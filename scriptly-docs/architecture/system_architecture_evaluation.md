...
* **재평가 및 핵심 문제점 (결합도 진단)**:
  * **단순 파일 분할의 한계**: 이전의 리팩토링은 주로 파일 라인 수를 줄이기 위해 마크업 블록을 단순 외부 파일로 쪼개는 방식(물리적 분할)에 그쳤습니다. 결과적으로 비즈니스 로직과 UI 프레젠테이션의 결합도가 해소되지 않은 채, 쪼개진 개별 파일들 역시 동일한 결합 문제를 갖고 있습니다.
  * **비즈니스 상태와 뷰의 혼재 (SRP 위반)**:
    * `login/page.tsx` & `register/page.tsx`: 이메일/비밀번호 등의 Form 폼 상태 제어(`useState`), 로그인/회원가입 REST API 연동(`api.post`), Axios 예외 처리, 그리고 라우터 및 토큰 세션 저장이 단일 파일 내부의 React 컴포넌트 안에 강하게 묶여 있습니다. 이로 인해 컴포넌트 자체가 비대해지며 UI 마크업 변경 시 비즈니스 흐름까지 영향받는 구조입니다.
    * `RightInsightPanel.tsx`: 기사 원문 및 Dramatic 분석 패널을 제공하지만, 내부에서 실시간 웹 크롤링 API 호출(`api.post`), 마우스 드래그 너비 조절 리사이즈 이벤트 리스너 등록/제거(`window.addEventListener`), 게이지바 및 사건 목록 렌더링 등 비동기 상태 제어와 로우 레벨 API 호출이 단일 컴포넌트 안에서 결합되어 있습니다.
  * **해결 방안 (획기적인 리팩토링 로드맵)**:
    * **Custom Hooks 패턴 전면 도입**: 비동기 데이터 통신(API), 입력 폼 관리 상태, 로컬 리사이즈 이벤트 등 모든 비즈니스 도메인 상태 관리 코드를 `useLoginForm.ts`, `useRegisterForm.ts`, `useRightInsight.ts`와 같이 순수 TypeScript 커스텀 훅으로 격리시킵니다.
    * **Presentational UI로의 전환**: 뷰 컴포넌트(예: `LoginForm.tsx`, `RegisterForm.tsx`, `RightInsightPanelPresenter.tsx`)는 상태를 직접 갖지 않거나 상태 변경 함수를 Props로 주입받아, 화면을 그리는 역할(마크업과 Tailwind CSS 렌더링)에만 집중하도록 만듭니다. 이를 통해 순수 마크업 컴포넌트로서 독립성과 가독성을 담보합니다.
