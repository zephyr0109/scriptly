          ...
          {activeInspiration ? (
            <div className="flex-grow flex flex-col gap-5 overflow-y-auto pr-1 min-h-0 custom-scrollbar-dark select-none">
              ...
              <div className="flex flex-col gap-5 border-t border-zinc-800/30 pt-4">
                ...
                {isSourceAnalyzing && !analyzedProjects[activeInspiration.id] && (
                  <div className={cn(
                ...
                {!isSourceAnalyzing && activeInspiration && activeInspiration.analysis_status === "FAILED" && !analyzedProjects[activeInspiration.id] && (
                  <div className={cn(
                ...
                {/* 2. 분석 대기 상태 */}
                {!isSourceAnalyzing && !analyzedProjects[activeInspiration.id] && activeInspiration.analysis_status !== "FAILED" && (
                  <div className={cn(
                    "p-8 rounded-2xl border text-center flex flex-col items-center gap-3 py-10",
                ...
                {/* 3. 분석 완료 리포트 노출 */}
                {!isAnalyzingQuick && analyzedProjects[activeInspiration.id] && (
                  <div className="flex flex-col gap-6 animate-in fade-in duration-500">
                    ...
                    <div className={cn(
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-black text-white">
                          <span className="text-amber-400 text-sm font-black">
                            {analyzedProjects[activeInspiration.id].score > 0 ? analyzedProjects[activeInspiration.id].score : "-"}
                          </span> / 100
                        </span>
                      </div>
                      <div className="w-full h-2 bg-zinc-800 rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${analyzedProjects[activeInspiration.id].score || 0}%` }}
                        />
                      </div>
                      <p className="text-[11px] text-zinc-455 leading-relaxed font-semibold">
                        {analyzedProjects[activeInspiration.id].scoreDesc}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className={cn(
                        ...
                        <div className="border-t border-zinc-800/30 pt-2 flex flex-col gap-1">
                          ...
                          <span className="text-white font-extrabold">{analyzedProjects[activeInspiration.id].recommendedVibe}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col gap-2 max-h-[250px] overflow-y-auto custom-scrollbar-dark pr-1">
                        {(analyzedProjects[activeInspiration.id].relatedPeople || []).map((person: any, idx: number) => (
                          <div 
                            key={idx}
                            className={cn(
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col gap-2">
                        {(analyzedProjects[activeInspiration.id].keyEvents || []).map((evt: string, idx: number) => (
